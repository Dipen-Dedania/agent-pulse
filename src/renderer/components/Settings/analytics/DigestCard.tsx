  ...
  return (
    <div className='flex-1 min-w-0'>
      ...
      {!hasAny ? (
        ...
        <>
          <p className='text-2xl font-bold text-strong leading-tight'>
            {formatDuration(digest.totalActiveMs)}
          </p>
          <p className='text-xs text-faint mt-0.5'>total active</p>
            ...
            {digest.perTool.map((t) => {
              ...
              return (
                <div key={t.toolId} className='flex items-center gap-2.5'>
                  ...
                  <div className='flex-1 min-w-0'>
                    <p className='text-xs font-medium text-primary truncate'>{meta?.label ?? t.toolId}</p>
                    {t.topTask && <p className='text-[10px] text-faint truncate'>{t.topTask}</p>}
                  </div>
                  <div className='text-right shrink-0'>
                    <p className='text-xs font-mono text-body'>{formatDuration(t.activeMs)}</p>
                    <p className='text-[10px] text-faint'>{t.sessions} {t.sessions === 1 ? 'session' : 'sessions'}</p>
                  </div>
                </div>
              );
            })}
          ...
          {digest.tokens && (
            <div className='mt-4 pt-3 border-t border-edge/60'>
              ...
              <div className='grid grid-cols-2 gap-x-3 gap-y-1 text-[11px] font-mono'>
                ...
                <span className='text-primary text-right'>{formatCompactNumber(digest.tokens.tokensOut)}</span>
                <span className='text-muted'>cache rd</span>
                <span className='text-primary text-right'>{formatCompactNumber(digest.tokens.cacheRead)}</span>
                <span className='text-muted'>cache wr</span>
                <span className='text-primary text-right'>{formatCompactNumber(digest.tokens.cacheWrite)}</span>
              </div>
    ...
    >
      {loading && !data ? (
        ...
        <div className='flex flex-col md:flex-row gap-6'>
          ...
          <div className='hidden md:block w-px bg-control/60' />
